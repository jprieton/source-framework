...
class Breadcrumb {
  ...
  private function add_crumbs_404() {
    $this->add_crumb( __( 'Error 404', \SourceFramework\TEXTDOMAIN ) );
  }
  ...
  private function add_crumbs_tag() {
    ...
    $this->add_crumb( sprintf( __( 'Posts tagged &ldquo;%s&rdquo;', \SourceFramework\TEXTDOMAIN ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
  }
  ...
  private function add_crumbs_date() {
    ...
    if ( is_day() ) {
      $this->add_crumb( get_the_time( 'd' ), get_day_link( get_the_time( 'Y' ), get_the_time( 'm' ), get_the_time( 'd' ) ) );
    }
  }
  ...
  private function add_crumbs_author() {
    ...
    $this->add_crumb( sprintf( __( 'Author: %s', \SourceFramework\TEXTDOMAIN ), $userdata->display_name ) );
  }
  ...
  private function search_trail() {
    if ( is_search() ) {
      $this->add_crumb( sprintf( __( 'Search results for &ldquo;%s&rdquo;', \SourceFramework\TEXTDOMAIN ), get_search_query() ), remove_query_arg( 'paged' ) );
    }
  }
  ...
  private function paged_trail() {
    if ( get_query_var( 'paged' ) ) {
      $this->add_crumb( sprintf( __( 'Page %d', \SourceFramework\TEXTDOMAIN ), get_query_var( 'paged' ) ) );
    }
  }
